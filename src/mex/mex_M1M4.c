 ...
 *mex_FMM.h.*/
#define STD_ERROR "Error in mex_M1M4: "
#define STD_WARN "Warning in mex_M1M4: "
#define SYNTAX_ERROR "incorrect syntax. \n\n usage : [M1,M4,maxlevel] = mex_M1M4(z,zp,dens,tol,levmax,numthreads)"
...
#define M4_C_OFFSET                (2*n_particles)
...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[]) {
    ...
    double *double_data;
    double *M1_re,*M1_im,*M4_re,*M4_im,*z_re,*z_im,*qa_c,*qb;
    double *M1_c,*M4_c,*C,*CC,*zp_re,*zp_im,*dens_re,*dens_im;
    double tol;
     ...
     * M4_c[2*n_particles]     Output field. Complex. Read/Write. Mutexed
     * QA_c[2*n_particles]     Temp density. Complex. Read-only
     * QB[n_particles]         Temp density. Pure imaginary. Read-only
    ...
    for(j = 0;j<n_particles;j++) {
        ...
        /*This is 2*imag(dens.*conj(zp)).*/
        qb[j] = 2*(dens_im[j]*zp_re[j] - dens_re[j]*zp_im[j]);
    }
    ...
    M4_re = mxGetPr(plhs[1]);
    M4_im = mxGetPi(plhs[1]);
    ...
    for(j = 0;j<n_particles;j++, ptr+=2) {
        M1_re[j] = -M1_c[ptr];
        M1_im[j] = -M1_c[ptr+1];
        M4_re[j] = (M4_c[ptr]-M1_c[ptr])/2;
        M4_im[j] = (M4_c[ptr+1]-M1_c[ptr+1])/2;
    }
...
THREAD_FUNC_TYPE MpolesWorker(void *argument) {
    ...
    double *M4_c = &(arg->double_data[M4_C_OFFSET]);
        ...
        *cursquare = i+1;
         ...
         *are used to speed it up.*/
         for(j=0;j<nparticles_in_box[current_box];j++) {
            ...
            _mm_store_pd(mpole2_c,_mm_sub_pd(_mm_load_pd(mpole2_c),zpow2));
            ...
            for(k=2;k<2*n_terms;k+=2) {
                ...
                /*Subtract zpow2 from mpole2.*/
                _mm_store_pd(&mpole2_c[k],_mm_sub_pd(_mm_load_pd(&mpole2_c[k]),zpow2));
            }
        }
               ...
               (current_box/nside + ilist_y[j+ci_flag*27])>=0) {
                ...
                if(nparticles_in_box[target_box] > taylor_threshold) {
                    ...
                    for(k=1;k<n_terms;k++){
                        ...
                        M4 = _mm_setr_pd(zsh_powers_c[2*k-2]*mpole2_c[2*k]-zsh_powers_c[2*k-1]*mpole2_c[2*k+1],
                        ...
                        for(l=0;l<n_terms;l++) {
                            ...
                            _mm_store_pd(&taylorexp2_c[2*l],_mm_add_pd(_mm_load_pd(&taylorexp2_c[2*l]),_mm_mul_pd(cc,M4)));
                        }
                    ...
                    if(nparticles_in_box[current_box] < 12) {
                        ...
                        for(k=0;k<(unsigned int)nparticles_in_box[target_box];k++) {
                            ...
                            for(l=0;l<(unsigned int)nparticles_in_box[current_box];l++) {
                                ...
                                M4_c[2*tptr2[k]] += (t3*tz_re-t2*tz_im)*tmp;
                                M4_c[2*tptr2[k]+1] += (t3*tz_im+t2*tz_re)*tmp;
                        ...
                        for(k=0;k<(unsigned int)nparticles_in_box[target_box];k++) {
                            ...
                            _mm_store_pd(&M4_c[2*tptr[k]], _mm_add_pd(tmp, _mm_load_pd(&M4_c[2*tptr[k]])));
                        }
                        UNLOCK_MUTEX(&output_mutex[target_box&(num_mutexes-1)]);
    ...
    for(;;) {
        ...
        *cursquare = i+1;
        ...
        for(j=0;j<nparticles_in_box[current_box];j++) {
            ...
            /*Add/Sub zpow2.*/
            tmp = _mm_addsub_pd(tmp, zpow2);
            _mm_store_pd(&M4_c[2*tptr[j]], _mm_add_pd(_mm_load_pd(&M4_c[2*tptr[j]]), tmp));
        }        
        ...
        for(j=0;j<nparticles_in_box[current_box];j++) {
            ...
            for(k=0;k<nparticles_in_box[current_box];k++) {
                if(k!=j) {
                    ...
                    M4_c[2*tptr[j]] += (t3*tz_re-t2*tz_im)*tmp;
                    M4_c[2*tptr[j]+1] += (t3*tz_im+t2*tz_re)*tmp;
                ...
                if(nparticles_in_box[source_box] > 0) {
                    ...
                    for(k=0;k<nparticles_in_box[current_box];k++) {
                        ...
                        for(l=0;l<nparticles_in_box[source_box];l++) {
                            ...
                            M4_c[2*tptr[k]] += (t3*tz_re-t2*tz_im)*tmp;
                            M4_c[2*tptr[k]+1] += (t3*tz_im+t2*tz_re)*tmp;
