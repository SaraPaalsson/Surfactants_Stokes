 ...
 *Matlab. At each particle z(k) the sum q(m)/(z(m)-z(k)) is computed over 
 *all m!=k, where q(m) are the charges of the particles.
 *  
...
#define TAYLOREXP_C_OFFSET         (2*n_terms)
#define ZSH_POWERS_C_OFFSET        (4*n_terms)
#define DBL_THDATA_SIZE            (6*n_terms)
...
void mexFunction(int nlhs,mxArray *plhs[],int nrhs,const mxArray *prhs[]) {
    ...
    double *double_data;
    double *M1_re,*M1_im,*q_re,*q_im,*z_re,*z_im;
    double *M1_c,*C,*CC;
    double tol;
     ...
     *to five(all inputs are required) and the number of outputs are one
     *or two(current_level is optional).*/
    ...
    /*Quit with error if q is not of class double*/
    if(!mxIsClass(prhs[1],"double"))
        mexErrMsgTxt(STD_ERROR Q_CLASS_ERROR);
    
    /*Quit with error if tol is not a double scalar*/
    if(mxGetM(prhs[2]) != 1 || mxGetN(prhs[2]) != 1 || !mxIsClass(prhs[2],"double"))
    ...
    lev_max = (int)mxGetScalar(prhs[3]);
    ...
    /*Quit with error if q is not a column vector*/
    if((mxGetM(prhs[1]) == 1 && mxGetM(prhs[1]) < mxGetN(prhs[1])) || 
       (mxGetM(prhs[1]) > 1 && mxGetN(prhs[1])>1))
        mexErrMsgTxt(STD_ERROR Q_DIM_ERROR);
        
    /*Quit with error if z is not the same size as q*/
    if(n_particles != mxGetM(prhs[1])) {
        mexErrMsgTxt(STD_ERROR Z_Q_SIZE_ERROR);
    }

    /*If z is empty then return an empty matrix and current_level = 0 if
     ...
     * --- One for each thread --- 
     * mpole_c[2*n_terms]      Temp data. Separated from other threads
     * taylorexp_c[2*n_terms]  Temp data. Separated from other threads
     * zsh_powers_c[2*n_terms] Temp data. Separated from other threads
     */
    double_data = _mm_mxMalloc((2*n_particles+n_terms*n_terms+(16-((n_terms*n_terms)&0xf))+numthreads*6*n_terms)*sizeof(double),16);
    ...
    /*Temporary pointers to the charges.*/
    q_re = mxGetPr(prhs[1]);
    q_im = mxGetPi(prhs[1]);
  
    /*The code assumes complex charges.*/
    if(q_re == NULL)
        q_re = mxCalloc(n_particles,sizeof(double));    
    if(q_im == NULL)
        q_im = mxCalloc(n_particles,sizeof(double));
    ...
    if(nlhs==2)
        plhs[1] = mxCreateDoubleScalar(current_level-1);
...
void Mpoles(double *z_re, double *z_im, double* q_re, double* q_im,
     ...
     *to all threads.*/
    if(ntboxes > 0)
        localexp_c = _mm_mxMalloc(2*n_terms*ntboxes*sizeof(double),16);
    else
        localexp_c = NULL;
        ...
        *cursquare = i+1;
         ...
         *are used to speed it up.*/
         for(j=0;j<nparticles_in_box[current_box];j++) {
            int k;
            __m128d zshift_re,zshift_im;
            /*The complex charge of this particle.*/
            __m128d zpow_2 = _mm_setr_pd(-q_re[particle_offsets[box_offsets[current_box]+j]],
                                         -q_im[particle_offsets[box_offsets[current_box]+j]]);
            /*Add zpow_2=-q to the first element of mpole, the first term
             *of the multipole expansion is minus the sum of charges.*/
            _mm_store_pd(mpole_c,_mm_add_pd(_mm_load_pd(mpole_c),zpow_2));
            /*The coordinate of the current particle with respect to the
             *center of this box.*/
            zshift_re = _mm_set1_pd(z_re[particle_offsets[box_offsets[current_box]+j]]-box_center_re);
            zshift_im = _mm_set1_pd(z_im[particle_offsets[box_offsets[current_box]+j]]-box_center_im);
            for(k=2;k<2*n_terms;k+=2) {
                /*We multiply zpow_2 by zshift here.*/
                /*Multiply zpow_2 by the real part of zshift*/
                __m128d tmp = _mm_mul_pd(zpow_2, zshift_re);
                /*Swap real and imaginary parts of zpow_2*/
                zpow_2 = _mm_shuffle_pd(zpow_2, zpow_2, 1);
                /*Mulitply the shuffled zpow_2 by the imaginary part of 
                 *zshift and add and subtract the products.*/
                zpow_2 = _mm_addsub_pd(tmp,_mm_mul_pd(zshift_im, zpow_2));
                /*The result of the multiplication is now in tmp, add it
                 *to mpole*/
                _mm_store_pd(&mpole_c[k],_mm_add_pd(_mm_load_pd(&mpole_c[k]),zpow_2));
            }
        }
            ...
            if((current_box%nside + ilist_x[j+ci_flag*27])<nside &&
               ...
               (current_box/nside + ilist_y[j+ci_flag*27])>=0) {
                ...
                if(nparticles_in_box[target_box] > taylor_threshold) {
                    ...
                    int k;
                    double* tptr2 = &localexp_c[2*taylor_box_nbr[target_box]*n_terms];
                    ...
                    /*Compute the local taylor expansion.*/
                    for(k=0;k<n_terms;k++){
                        taylorexp_c[2*k] = mpole_c[0]*CC[k];
                        taylorexp_c[2*k+1] = mpole_c[1]*CC[k];
                    }
                    ...
                    for(k=1;k<n_terms;k++){
                        ...
                        t1 = zsh_powers_c[2*k-2]*mpole_c[2*k]-zsh_powers_c[2*k-1]*mpole_c[2*k+1];
                        t2 = zsh_powers_c[2*k-2]*mpole_c[2*k+1]+zsh_powers_c[2*k-1]*mpole_c[2*k];

                        /*We spend a lot of time here, so use SIMD.*/
                        t = _mm_setr_pd(t1,t2);
                        for(l=0;l<(unsigned int)n_terms;l++) {
                            __m128d cc = _mm_set1_pd(tptr[l]);
                            __m128d res = _mm_load_pd(&taylorexp_c[2*l]);                          
                            _mm_store_pd(&taylorexp_c[2*l],_mm_add_pd(res,_mm_mul_pd(cc,t)));
                        }
                    ...
                    for(k=0;k<2*n_terms;k+=2) {
                        /*Multiply zsh_powers_c[k] by taylorexp_c[k]*/
                        __m128d z0p = _mm_load_pd(&zsh_powers_c[k]);
                        /*Multiply z0p by the real part of taylorexp_c*/
                        __m128d tmp = _mm_mul_pd(z0p, _mm_set1_pd(taylorexp_c[k]));
                        /*Swap real and imaginary parts of z0p*/
                        z0p = _mm_shuffle_pd(z0p, z0p, 1);
                        /*Mulitply z0p by the imaginary part of taylorexp_c 
                         *and add and subtract the products.*/
                        z0p = _mm_addsub_pd(tmp, _mm_mul_pd(_mm_set1_pd(taylorexp_c[k+1]), z0p));
                        _mm_store_pd(&tptr2[k], _mm_add_pd(_mm_load_pd(&tptr2[k]), z0p));
                    }
                    ...
                    if(nparticles_in_box[current_box] < 12) {
                        ...
                        for(k=0;k<(unsigned int)nparticles_in_box[target_box];k++) {
                            ...
                            for(l=0;l<(unsigned int)nparticles_in_box[current_box];l++) {
                                __m128d z = _mm_setr_pd(z_re[tptr[l]]-z_re[tptr2[k]],
                                                        z_im[tptr[l]]-z_im[tptr2[k]]);
                                __m128d denom = _mm_mul_pd(z, z);
                                __m128d t = _mm_mul_pd(z, _mm_set1_pd(q_im[tptr[l]]));
                                
                                denom = _mm_hadd_pd(denom, denom);
                                z = _mm_shuffle_pd(z, z, 1);
                                z = _mm_addsub_pd(t, _mm_mul_pd(_mm_set1_pd(q_re[tptr[l]]), z));
                                z = _mm_div_pd(_mm_shuffle_pd(z, z, 1), denom);
                                tmp = _mm_add_pd(tmp, z);
                            }
                            _mm_store_pd(&M1_c[2*tptr2[k]], _mm_add_pd(_mm_load_pd(&M1_c[2*tptr2[k]]), tmp));
                        ...
                        /*Evaluate the multipole expansion using Horner's rule.
                        ...
                        for(k=0;k<(unsigned int)nparticles_in_box[target_box];k++) {
                            ...
                            __m128d tmp2;
                            __m128d zshift_re = _mm_set1_pd(t1*tmpd);
                            __m128d zshift_im = _mm_set1_pd(-t2*tmpd);
                            
                            __m128d zpow_2 = _mm_load_pd(&mpole_c[2*n_terms-2]);
                            /*This is the main hotspot in shark. We spend
                             *over 30% of the time here.*/
                            for(m=2*(n_terms-2);m>=0;m-=2) {
                                /*We multiply zpow_2 by zshift here. Do it via SSE3*/
                                /*Multiply zpow_2 by the real part of zshift*/
                                __m128d tmp = _mm_mul_pd(zpow_2, zshift_re);
                                /*Swap real and imaginary parts of zpow_2*/
                                zpow_2 = _mm_shuffle_pd(zpow_2, zpow_2, 1);
                                /*Mulitply zpow_2 by the imaginary part of zshift and
                                 *Add and subtract the products.*/
                                zpow_2 = _mm_addsub_pd(tmp, _mm_mul_pd(zshift_im, zpow_2));
                                /*Add a term of the multipole expansion to the result*/
                                zpow_2 = _mm_add_pd(_mm_load_pd(&mpole_c[m]), zpow_2);
                            }
                            /*Multiply the result of the above loop by zshift*/
                            /*Multiply zpow_2 by the real part of zshift*/
                            tmp2 = _mm_mul_pd(zpow_2, zshift_re);
                            /*Swap real and imaginary parts of zpow_2*/
                            zpow_2 = _mm_shuffle_pd(zpow_2, zpow_2, 1);
                            /*Mulitply zpow_2 by the imaginary part of zshift and
                             *Add and subtract the products.*/
                            zpow_2 = _mm_addsub_pd(tmp2, _mm_mul_pd(zshift_im, zpow_2));
                            /*Store the result in a temporary array of size
                             *n_terms*2.*/
                            _mm_store_pd(&temp_c[2*k], zpow_2);
                        }
                        ...
                        LOCK_MUTEX(&output_mutex[target_box&(num_mutexes-1)]);
                        for(k=0;k<(unsigned int)nparticles_in_box[target_box];k++) {
                            __m128d tmp = _mm_load_pd(&temp_c[2*k]);
    ...
    for(;;) {
        ...
        *cursquare = i+1;
        ...
        for(j=0;j<nparticles_in_box[current_box];j++) {
            ...
            __m128d zpow_2 = _mm_load_pd(&localexp_c[2*(n_terms-1+i*n_terms)]);
            ...
            for(m=n_terms-2;m>=0;m--) {
                 ...
                 *Add and subtract the products.*/
                zpow_2 = _mm_addsub_pd(tmp, _mm_mul_pd(zshift_im, zpow_2));
                /*Add localexp to the result*/
                zpow_2 = _mm_add_pd(_mm_load_pd(&localexp_c[2*(m+i*n_terms)]), zpow_2);
            }
            ...
            _mm_store_pd(&M1_c[2*tptr[j]], _mm_add_pd(_mm_load_pd(&M1_c[2*tptr[j]]), zpow_2));
        ...
        for(j=0;j<nparticles_in_box[current_box];j++) {
            ...
            for(k=0;k<nparticles_in_box[current_box];k++) {
                if(k!=j) {
                    ...
                    double tmp = (double)1/(tz_re*tz_re+tz_im*tz_im);
                    M1_c[2*tptr[j]] += (q_re[tptr[k]]*tz_re 
                                             + q_im[tptr[k]]*tz_im)*tmp;
                    M1_c[2*tptr[j]+1] += (q_im[tptr[k]]*tz_re 
                                             - q_re[tptr[k]]*tz_im)*tmp;
                }
            }
        }
                ...
                if(nparticles_in_box[source_box] > 0) {
                    ...
                    for(k=0;k<nparticles_in_box[current_box];k++) {
                        ...
                        for(l=0;l<nparticles_in_box[source_box];l++) {
                            __m128d z = _mm_setr_pd(z_re[tptr2[l]]-z_re[tptr[k]],
                                                    z_im[tptr2[l]]-z_im[tptr[k]]);
                            __m128d denom = _mm_mul_pd(z,z);
                            __m128d t = _mm_mul_pd(z,_mm_set1_pd(q_im[tptr2[l]]));
                            
                            denom = _mm_hadd_pd(denom,denom);
                            z = _mm_shuffle_pd(z, z, 1);
                            z = _mm_addsub_pd(t, _mm_mul_pd(_mm_set1_pd(q_re[tptr2[l]]), z));
                            z = _mm_div_pd(_mm_shuffle_pd(z, z, 1),denom);
                            tmp = _mm_add_pd(tmp,z);
                         
                        }
                        _mm_store_pd(&M1_c[2*tptr[k]], _mm_add_pd(_mm_load_pd(&M1_c[2*tptr[k]]), tmp));
